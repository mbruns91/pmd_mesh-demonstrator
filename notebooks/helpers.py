...
def query_instance(name: str,
    ...
    results = {}
    name = re.sub(r'[^a-zA-Z0-9_]', '_', name) # remove all invalid characters for python identifiers (everything not a letter, number or underscores) 
    if name not in results:
        results[name] = {}
    for ep in endpoints:
        if any(substring in ep for substring in datasets): # only query endpoints whose address contains any of the dataset names we passed
